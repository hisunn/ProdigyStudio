...
class Calendar {
  ...
  function save ($start, $end, $txt, $color, $id=null) {
    ...
    if ($id==null) {
      $sql = "INSERT INTO `schedule` (`evt_start`, `evt_end`, `evt_text`, `evt_color`) VALUES (?,?,?,?)";
      $data = [$start, $end, $txt, $color];
    } else {
      $sql = "UPDATE `schedule` SET `evt_start`=?, `evt_end`=?, `evt_text`=?, `evt_color`=? WHERE `evt_id`=?";
  ...
  // (E) DELETE EVENT
  function del ($id) {
    return $this->exec("DELETE FROM `schedule` WHERE `evt_id`=?", [$id]);
  }

  // (F) GET schedule FOR SELECTED MONTH
  function get ($month, $year) {
    ...
    while ($row = $this->stmt->fetch()) {
      ...
      for ($d=$eStartDay; $d<=$eEndDay; $d++) {
        if (!isset($schedule["d"][$d])) { $schedule["d"][$d] = []; }
        $schedule["d"][$d][] = $row["evt_id"];
      }
      $schedule["e"][$row["evt_id"]] = $row;
      $schedule["e"][$row["evt_id"]]["first"] = $eStartDay;
    }
    return $schedule;
  }
}
